...
export class ThreeStructureRenderer {
	...
	constructor(
		...
		if (this.debug) {
			...
			console.log('[lodestone] atlas info', {
	...
	private createStructureMaterial(transparent: boolean) {
		return new THREE.RawShaderMaterial({
			name: transparent ? 'lodestone-structure-transparent' : 'lodestone-structure-opaque',
			vertexShader: `
	...
	private createColoredMaterial() {
		return new THREE.RawShaderMaterial({
			name: 'lodestone-structure-colored',
			vertexShader: `
				precision highp float;
				uniform mat4 projectionMatrix;
				uniform mat4 modelViewMatrix;
	...
	private createInvisibleBlocks() {
		...
		if (volume > 200_000) {
			if (this.debug) {
				console.warn('[lodestone] Skipping invisible blocks buffer for large structure', { volume })
			}
			return undefined
		}
